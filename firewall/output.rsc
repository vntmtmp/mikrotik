...
# FORWARD - General
add action=drop chain=forward comment="Drop IP yang diblokir" src-address-list=Blocked
add action=drop chain=forward comment="Log dan drop traffic malware" dst-address-list=Malware log=yes log-prefix="OUT MALWARE:"
add action=accept chain=forward comment="Lewati firewall untuk IP yang diizinkan" dst-address-list=Allowed
